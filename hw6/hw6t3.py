...
def printPriceList(list, size):
    if size == len(list):
        print("прайс-лист:")
    else:
        print(f'Прайс-лист {size} самых дорогих товаров:')
    for i in range(size):
...
def createPriceList(size):
    ...
    for i in range(0, random.randint(int(size / 10), int(size / 7))):
        price = round(Decimal(random.uniform(5, size)),
                      2)  # float - головная боль для таких случаев, использовал стороннюю библиотеку.
...
printPriceList(prices, len(prices))
print(
...
printPriceList(sorted(prices), len(prices))
print("Объект списка после сортировки для доказательства, что я не создавал новый список - ✓\n", id(prices))
prices2 = sorted(prices, reverse=True)
print(
    "Создать новый список, содержащий те же цены, но отсортированные по убыванию. - ✓. Доказательство - другая ссылка\n",
    id(prices2))
print("Отсортированный по убыванию ", end='')
printPriceList(prices2, len(prices2))
printPriceList(prices2, 5)